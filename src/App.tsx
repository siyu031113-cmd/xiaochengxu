    ...
    return (
      <div className="flex flex-col h-full bg-blue-50 overflow-y-auto">
        <div className="flex-1 p-8 flex flex-col justify-center">
            <div className="w-20 h-20 bg-blue-600 rounded-3xl flex items-center justify-center shadow-xl shadow-blue-200 mb-6 mx-auto"><span className="text-4xl text-white"><Plane className="text-white transform -rotate-45" size={40}/></span></div>
            <div className="text-center mb-10"><h1 className="text-2xl font-bold text-slate-900">{BRAND_NAME_EN}</h1><p className="text-blue-600 font-medium">{BRAND_NAME_CN} · 赴美带薪实习</p></div>
            {isRegister ? (
    ...
    <div className="p-4 space-y-5 pb-24 h-full overflow-y-auto bg-slate-50">
      ...
      <div className="space-y-4">
        {filteredJobs.length === 0 ? (
            ...
            return (
            <div key={job.id} onClick={() => { if (activeApp && activeApp.jobId !== job.id) { alert("您当前已有申请中的岗位，无法查看其他岗位。"); return; } setSelectedJobId(job.id); }} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden active:scale-[0.98] transition-transform cursor-pointer group">
                ...
                {myApps.length === 0 ? (<div className="text-center py-10 text-slate-400">No applications yet.</div>) : (myApps.map(app => {
                        ...
                        return (
                            <div key={app.id} onClick={() => setSelectedJobId(job.id)} className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 active:scale-[0.98] transition-transform cursor-pointer group relative overflow-hidden">
                                ...
                                {app.status === 'approved' && (<div className="mt-4 bg-slate-50 rounded-xl p-3" onClick={(e) => e.stopPropagation()}><div className="text-xs font-bold text-slate-500 mb-2 flex items-center"><ClipboardList size={14} className="mr-1"/> Weekly Check-in</div><div className="grid grid-cols-2 gap-2">{['Work is great', 'Need Housing Help', 'Job Issues', 'Other'].map(opt => (<button key={opt} onClick={(e) => handleSubmitFeedback(e, job.id, opt)} className="py-2 bg-white border border-slate-200 rounded-lg text-xs font-medium text-slate-600 hover:border-blue-400 hover:text-blue-600 active:bg-blue-50">{opt}</button>))}</div></div>)}
                            </div>
                        );
                    }))}
            </div>
          ...
          <div className="space-y-4">
               ...
               <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                   ...
                   <div className="grid grid-cols-2 gap-4">
                       <div><label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Year</label><div className="relative"><select value={newJob.programYear} onChange={e => setNewJob({...newJob, programYear: e.target.value})} className="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm font-bold py-3 px-3 rounded-xl outline-none appearance-none">{['2023','2024','2025','2026'].map(y => <option key={y} value={y}>{y}</option>)}</select><ChevronRight className="absolute right-3 top-1/2 -translate-y-1/2 rotate-90 text-slate-400 pointer-events-none" size={16}/></div></div>
                       <div><label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Housing Cost</label><input className="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm font-bold py-3 px-3 rounded-xl outline-none" value={newJob.housing || ''} placeholder="$150/wk" onChange={e => setNewJob({...newJob, housing: e.target.value})} /></div>
                       <div><label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Start Dates</label><input className="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm font-bold py-3 px-3 rounded-xl outline-none" value={newJob.startDateRange || ''} placeholder="Jun 15 - Jun 30" onChange={e => setNewJob({...newJob, startDateRange: e.target.value})} /></div>
  ...
  const renderJobDetail = () => {
    ...
    return (
        <div className="absolute inset-0 bg-white z-50 flex flex-col animate-in slide-in-from-bottom-full duration-300">
             {/* --- CUTE PASSPORT STAMP WATERMARK --- */}
             <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none overflow-hidden">
                 <Cloud className="absolute top-20 left-10 text-blue-100 opacity-50" size={80} />
                 <Cloud className="absolute bottom-40 right-10 text-blue-100 opacity-50" size={100} />
                 <Plane className="absolute top-1/3 right-20 text-blue-200 opacity-30 -rotate-45" size={120} />
                 <div className="border-4 border-blue-200 rounded-full p-10 rotate-[-15deg] opacity-20 flex flex-col items-center justify-center w-80 h-80">
                    <div className="text-center">
                        <Plane className="inline-block mb-2 text-blue-400" size={40} />
                        <div className="text-xs font-bold text-blue-400 tracking-[0.3em] uppercase mb-1">Official Program</div>
                        <div className="text-xl font-black text-blue-900 uppercase leading-none">Blueprint</div>
                        <div className="text-xl font-black text-blue-900 uppercase leading-none mb-1">Global</div>
                        <div className="text-3xl font-black text-blue-600 font-serif">蓝途启航</div>
                        <div className="flex justify-center space-x-2 mt-2"><Star className="text-yellow-400 fill-yellow-400" size={16} /><Star className="text-yellow-400 fill-yellow-400" size={16} /><Star className="text-yellow-400 fill-yellow-400" size={16} /></div>
                    </div>
                 </div>
             </div>
