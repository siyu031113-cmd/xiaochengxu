...
const App: React.FC = () => {
  ...
  const handleAddCustomContact = () => {
    setEmergencyForm(prev => ({
        ...prev,
        others: [...(prev.others || []), { id: Date.now().toString(), name: '', phone: '', role: '' }]
    }));
  };
  ...
  const handleRemoveCustomContact = (id: string) => {
    setEmergencyForm(prev => ({
        ...prev,
        others: (prev.others || []).filter(c => c.id !== id)
    }));
  };
  ...
  const handleUpdateCustomContact = (id: string, field: keyof CustomContact, value: string) => {
    setEmergencyForm(prev => ({
        ...prev,
        others: (prev.others || []).map(c => c.id === id ? { ...c, [field]: value } : c)
    }));
  };
  ...
  const handleSaveGuide = () => {
      if (!editingGuide || !editingGuide.title || !editingGuide.content) {
          alert("Please enter title and content");
          return;
      }
      
      const newGuide: Guide = {
          id: editingGuide.id || `g${Date.now()}`,
        ...
        <div className="grid grid-cols-2 gap-3 mb-2">
            <button onClick={() => setActiveTab('admin-guides')} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-center space-x-2 active:scale-95 transition-transform">
                ...
                <div className="text-left">
                    <div className="text-xs text-slate-400 font-bold uppercase">Manage</div>
                    <div className="text-sm font-bold text-slate-800">Guides</div>
                </div>
            </button>
             <button onClick={() => setActiveTab('post-job')} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-center space-x-2 active:scale-95 transition-transform">
                <div className="w-10 h-10 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center">
                    <Plus size={20} />
                </div>
                <div className="text-left">
                    <div className="text-xs text-slate-400 font-bold uppercase">Post</div>
                    <div className="text-sm font-bold text-slate-800">New Job</div>
                </div>
            </button>
        </div>
      ...
      return (
          <div className="p-4 space-y-5 pb-24 overflow-y-auto h-full bg-slate-50">
              <header className="px-1 pt-2 flex justify-between items-end">
                  <div>
                      <div className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Admin Console</div>
                      <h2 className="text-2xl font-bold text-slate-900">Pre-Departure Guides</h2>
                  </div>
                  <button 
                      onClick={() => setEditingGuide({ title: '', content: '' })}
                      className="bg-blue-600 text-white px-3 py-2 rounded-xl text-sm font-bold flex items-center shadow-lg shadow-blue-200"
                  >
              ...
              <div className="space-y-3">
                  {db.guides.map(guide => (
                      <div key={guide.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center">
                          <div className="flex items-center">
                              <div className="w-10 h-10 rounded-full bg-indigo-50 text-indigo-500 flex items-center justify-center mr-3">
                                  <FileText size={20} />
                              </div>
                              <div>
                                  <div className="font-bold text-slate-800">{guide.title}</div>
                                  <div className="text-xs text-slate-400">Updated: {guide.updateDate}</div>
                              </div>
                          </div>
                          <div className="flex space-x-2">
                              <button onClick={() => setEditingGuide(guide)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg">
                                  <Edit2 size={16} />
                              </button>
                              <button onClick={() => handleDeleteGuide(guide.id)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg">
                                  <Trash2 size={16} />
                              </button>
                          </div>
                      </div>
                  ...
                  {db.guides.length === 0 && (
                      <div className="text-center py-10 text-slate-400">No guides found. Add one to help students!</div>
                  )}
              </div>
          </div>
      );
          ...
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-3xl w-full h-[85%] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                  <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white">
                      <h3 className="font-bold text-slate-900">{editingGuide.id ? 'Edit Guide' : 'New Guide'}</h3>
                      <button onClick={() => setEditingGuide(null)} className="p-2 bg-slate-100 rounded-full text-slate-500"><X size={20}/></button>
                  </div>
                  <div className="p-4 flex-1 overflow-y-auto space-y-4">
                      <div>
                          <label className="text-xs font-bold text-slate-400 uppercase block mb-1">Title</label>
                          <input 
                              className="w-full text-lg font-bold border-b border-slate-200 py-2 outline-none focus:border-blue-500 transition-colors"
                              placeholder="e.g. Visa Checklist"
                              value={editingGuide.title}
                              onChange={e => setEditingGuide({...editingGuide, title: e.target.value})}
                          />
                      </div>
                      <div className="h-full flex flex-col">
                           <label className="text-xs font-bold text-slate-400 uppercase block mb-2">Content (Markdown)</label>
                           <textarea 
                              className="w-full flex-1 bg-slate-50 rounded-xl p-4 text-sm leading-relaxed outline-none border border-transparent focus:bg-white focus:border-blue-200 resize-none transition-colors"
                              placeholder="Write your guide content here... Use # for headers, - for lists."
                  ...
                  <div className="p-4 border-t border-slate-100 bg-slate-50">
                      <button onClick={handleSaveGuide} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-blue-200">
                          Save Guide
                      </button>
                  </div>
              </div>
      ...
      return (
          <div className="absolute inset-0 z-50 bg-white flex flex-col animate-in slide-in-from-right duration-300">
              <div className="h-16 flex items-center px-4 border-b border-slate-100 sticky top-0 bg-white z-10">
                  <button onClick={() => setViewingGuide(null)} className="p-2 -ml-2 hover:bg-slate-50 rounded-full transition-colors">
                      <ChevronRight className="rotate-180 text-slate-600" size={24}/>
                  </button>
                  <span className="font-bold text-slate-900 ml-2">Guide Details</span>
              </div>
              <div className="flex-1 overflow-y-auto p-6">
                  <h1 className="text-2xl font-bold text-slate-900 mb-2">{viewingGuide.title}</h1>
                  <div className="text-xs text-slate-400 mb-8">Last Updated: {viewingGuide.updateDate}</div>
    ...
    <div className="p-4 space-y-5 pb-24 h-full overflow-y-auto bg-slate-50">
      ...
      <div className="space-y-4">
        {db.jobs
            ...
            <div key={job.id} 
                onClick={() => {
                    if (activeApp && activeApp.jobId !== job.id) {
                        alert("一次只能申请一个岗位，如果开始申请了，其他岗位则不能查看。");
                        return;
                    }
                    setSelectedJobId(job.id);
                }} 
                className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden active:scale-[0.98] transition-transform cursor-pointer group"
            >
                <div className="h-32 bg-gray-200 relative">
                    ...
                    {activeApp && activeApp.jobId !== job.id && (
                        <div className="absolute inset-0 bg-slate-50/80 flex items-center justify-center text-slate-500 font-bold backdrop-blur-[1px]">
                            <span className="flex items-center text-xs">Locked (Active Application)</span>
                        </div>
                    )}
                </div>
                <div className="p-4">
    ...
    <div className="flex flex-col h-screen bg-slate-50 overflow-hidden font-sans text-slate-900 mx-auto max-w-md shadow-2xl relative">
      ...
      {db.currentUser && !selectedJobId && !adminDetailJobId && activeTab !== 'emergency' && (
        <TabBar 
            ...
            activeTab={activeTab === 'emergency' ? 'services' : activeTab} 
